...
Word::Word(const char* tmp)
{
	...
	for (int i = 0; i < Size_of_Word; i++) { word[i] = tmp[i]; } 
	word[Size_of_Word] = '\0';
}
...
char & Word::operator[](int n)
{
	if (word)
	{
		n %= Size_of_Word;  // if n is biger the the length of the word
		return word[n];
	}
	else
	{
...
Word & Word::operator+=(int x)
{
	if (Size_of_Word)
	{
		...
		x %= Size_of_Word;// if n is biger the the length of the word
...
Word & Word::operator-=(int x)
{
	...
	if (word)
	{
		...
		x %= (Size_of_Word - 1);// if n is biger the the length of the word
